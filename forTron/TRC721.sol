...
contract TRC721Token is TRC721, TRC721Enumerable, TRC721MetadataMintable {
    ...
    constructor(address newOwner) public TRC721Metadata("GunGirls", "GNG") {
        creator = newOwner;
        _setBaseURI(
            "https://gateway.pinata.cloud/ipfs/QmcCnCPnptuxd8b7FWvRuqBMXbxuyVKopp4fTSiXdwUXPU/"
        );
        for (uint256 i = 1; i < 9; i++) {
            string memory tokenURI = uint2str(i);
            mintWithTokenURI(msg.sender, i, tokenURI);
        }
    }
